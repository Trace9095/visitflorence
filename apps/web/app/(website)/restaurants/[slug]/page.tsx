...
import { MapPin, Phone, Globe, Clock, ExternalLink, Utensils, Star, Navigation } from "lucide-react";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const [listing] = await db
    .select()
    .from(listings)
    .where(and(eq(listings.slug, slug), eq(listings.category, "restaurants")))
    .limit(1);

  if (!listing) return { title: "Not Found" };

  return {
    title: `${listing.name} | Florence CO Restaurants`,
    description: listing.shortDescription,
    openGraph: {
      title: `${listing.name} | Dining in Florence CO`,
      description: listing.shortDescription,
      type: "website",
    },
    twitter: {
        ...
        <div className="relative z-10 max-w-5xl mx-auto w-full px-4 pb-6">
          <nav className="text-xs mb-3" aria-label="breadcrumb">
            <ol className="flex items-center gap-1.5 flex-wrap" style={{ color: "#9B8374" }}>
              ...
              <li>/</li>
              <li><Link href="/restaurants" style={{ color: "#9B8374" }} className="hover:opacity-80">Restaurants</Link></li>
              <li>/</li>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              <Utensils size={16} style={{ color: "#C19A6B" }} />
              <span className="text-xs font-mono font-semibold uppercase tracking-widest"
                style={{ color: "#C19A6B" }}>About This Restaurant</span>
            </div>
          ...
          <div className="rounded-2xl p-6 border" style={{ backgroundColor: "#2C1A0E", borderColor: "#3D2518" }}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-mono font-semibold uppercase tracking-widest"
                style={{ color: "#C19A6B" }}>Florence — Closest Town to the Royal Gorge</span>
            </div>
            <p className="text-sm leading-6 mb-4" style={{ color: "#9B8374" }}>
              Dine in Florence, then head just 15 minutes to one of Colorado&apos;s most spectacular natural landmarks.
            </p>
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              <ExternalLink size={16} style={{ color: "#C19A6B" }} />
              <h2 className="text-xs font-mono font-semibold uppercase tracking-widest"
                style={{ color: "#C19A6B" }}>More Epic Experiences Nearby</h2>
            </div>
          ...
          {listing.tier === "free" && (
            <div className="rounded-2xl p-6 border text-center"
              ...
              <Utensils size={28} className="mx-auto mb-3" style={{ color: "#C19A6B" }} />
              <h3 className="font-bold text-lg mb-2" style={{ color: "#F5EDD6" }}>Own {listing.name}?</h3>
              <p className="text-sm leading-6 mb-4" style={{ color: "#9B8374" }}>
                Upgrade to Premium to feature your menu, gallery, specials, and reservation link.
              </p>
              <Link href="/pricing"
          ...
          <div className="flex items-center justify-between pt-2 border-t" style={{ borderColor: "#3D2518" }}>
            <span className="text-xs" style={{ color: "#9B8374" }}>Is this your restaurant?</span>
        ...
        <div>
          <div className="rounded-2xl border p-5 space-y-4 sticky top-6"
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <div className="text-xs font-mono uppercase tracking-wide mb-0.5" style={{ color: "#9B8374" }}>Address</div>
            ...
            {listing.hours && (
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="text-xs font-mono uppercase tracking-wide mb-0.5" style={{ color: "#9B8374" }}>Hours</div>
            ...
            {listing.menuUrl && (
              <a href={listing.menuUrl} target="_blank" rel="noopener noreferrer"
            ...
            {listing.bookingUrl && (
              <a href={listing.bookingUrl} target="_blank" rel="noopener noreferrer"
                className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl font-semibold text-sm min-h-[44px] border transition-colors hover:opacity-80"
                style={{ borderColor: "#C19A6B", color: "#C19A6B" }}>
                Make a Reservation
              </a>
            )}
