...
import { MapPin, Phone, Globe, Clock, ExternalLink, Beer, Utensils, Star, Navigation } from "lucide-react";
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const [listing] = await db
    .select()
    .from(listings)
    .where(and(eq(listings.slug, slug), eq(listings.category, "breweries")))
    .limit(1);

  if (!listing) return { title: "Not Found" };

  return {
    title: `${listing.name} | Florence CO Breweries`,
    description: listing.shortDescription,
    openGraph: {
      title: `${listing.name} | Craft Beer in Florence CO`,
      description: listing.shortDescription,
      type: "website",
    },
    twitter: {
        ...
        <div className="relative z-10 max-w-5xl mx-auto w-full px-4 pb-6">
          <nav className="text-xs mb-3" aria-label="breadcrumb">
            <ol className="flex items-center gap-1.5 flex-wrap" style={{ color: "#9B8374" }}>
              ...
              <li>/</li>
              <li><Link href="/breweries" style={{ color: "#9B8374" }} className="hover:opacity-80">Breweries</Link></li>
              <li>/</li>
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              <Beer size={16} style={{ color: "#C19A6B" }} />
              <span className="text-xs font-mono font-semibold uppercase tracking-widest"
                style={{ color: "#C19A6B" }}>About This Brewery</span>
            </div>
          ...
          <div className="rounded-2xl p-6 border" style={{ backgroundColor: "#2C1A0E", borderColor: "#3D2518" }}>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-mono font-semibold uppercase tracking-widest"
                style={{ color: "#C19A6B" }}>Gateway to the Royal Gorge</span>
            </div>
            <p className="text-sm leading-6 mb-4" style={{ color: "#9B8374" }}>
              Florence is the closest town to the Royal Gorge — grab a craft beer here, then head out for adventure.
            </p>
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-xs font-mono font-semibold uppercase tracking-widest"
                style={{ color: "#C19A6B" }}>Plan Your Full Florence Adventure</h2>
            </div>
          ...
          {listing.tier === "free" && (
            <div className="rounded-2xl p-6 border text-center"
              ...
              <Beer size={28} className="mx-auto mb-3" style={{ color: "#C19A6B" }} />
              <h3 className="font-bold text-lg mb-2" style={{ color: "#F5EDD6" }}>Own {listing.name}?</h3>
              <p className="text-sm leading-6 mb-4" style={{ color: "#9B8374" }}>
                Upgrade to Premium to add your full menu, event calendar, gallery, booking link, and more.
              </p>
              <Link href="/pricing"
          ...
          <div className="flex items-center justify-between pt-2 border-t" style={{ borderColor: "#3D2518" }}>
            <span className="text-xs" style={{ color: "#9B8374" }}>Is this your brewery?</span>
